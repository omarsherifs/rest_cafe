      ...
      builder: (context, state) {
        ...
        return Scaffold(
          ...
          body: SafeArea(
            child: Container(
              padding:
                  EdgeInsets.only(top: 20.0.sp, left: 20.0.sp, right: 20.0.sp),
              child: state is TypesLoading
                  ? Center(child: CircularProgressIndicator())
                  : state is TypesFailed
                      ? Center(
                          child: Text("اعد المحاولة لاحقا"),
                        )
                              ...
                              Container(
                                ...
                                child: ListView.separated(
                                    ...
                                    separatorBuilder: (context, index) =>
                                        SizedBox(width: 10),
                                    itemCount:
                                        HomeCubit.get(context).types.length),
                              ),
                                ...
                                child: ListView.separated(
                                    ...
                                    itemBuilder: (context, index) => InkWell(
                                        onTap: () {
                                          navigateTo(
                                              context,
                                              BranchesScreen(resturants[index]
                                                  .id
                                                  .toString()));
                                        },
                          ...
                          onPressed: () async {
                            ...
                            if (isFav) {
                              ...
                              await DioHelper.postData(
                                  endpoint:
                                      "api/v1/branch/favourites/${data[index!].id}",
                                  context: context);
                            } else {
                              data[index!].IsFavourite = false;
                              await DioHelper.delete(
                                  endpoint:
                                      "api/v1/branch/favourites/remove/${data[index!].id}",
                                  context: context);
                            }
                          },
                        ),
...
class labolOfFristListView extends StatelessWidget {
  ...
  const labolOfFristListView(
      {Key? key, required this.path, required this.text, required this.index})
      : super(key: key);
